...
def fit(model, data, n_epochs, opt, crit, metrics=None, callbacks=None, stepper=Stepper,
    ...
    for epoch in tnrange(tot_epochs, desc='Epoch'):
        time.sleep(3.0)  # to make 'Epoch' progress bar visible for a little while'
        ...
        t = tqdm(iter(cur_data.trn_dl), leave=False, total=num_batch, desc=f'Epoch({epoch}/{tot_epochs})')
